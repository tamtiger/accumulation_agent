...
- **Chức năng tính toán:**
    - `avg_cost_fifo_lot`: Giá vốn của lô cụ thể ở đầu hàng đợi FIFO (sử dụng cho cổng chặn bán `INV-5`).
    - `avg_cost_portfolio`: Giá vốn trung bình có trọng số của tất cả các lô còn lại (sử dụng trong biểu diễn trạng thái và báo cáo).
    - `realized_pnl_btc`, `unrealized_pnl_btc`: Lợi nhuận BTC đã thực hiện và chưa thực hiện.
...
- Sẵn sàng xuất dữ liệu báo cáo thuế (xem chi tiết tại Phần 19).
...
| `A_vol` | ATR(14) / giá hiện tại | Co dãn khoảng cách lưới thích ứng (Chỉ là giá trị đại diện; sử dụng σ_ann để định quy mô lưới) |
| `A_cost` | Giá vốn theo lô FIFO | Chặn bán (INV-5) |
| `A_mean` | EMA(20, khung 4 giờ) | Mốc hồi quy trung bình (Mean-reversion anchor) |
| `A_local_low` | Giá thấp nhất trong 48 giờ qua - Rolling min(low, 48h) | Mốc tham chiếu kích hoạt nhịp hồi phục — xem Phần 10 |
...
Khoảng cách lưới thích ứng với biến động thực tế. Sử dụng **độ biến động thực tế được chuẩn hóa theo năm trong 30 ngày (σ_ann)** cho khoảng cách lưới, chứ không sử dụng `A_vol` thô (tỷ lệ ATR).

```
σ_ann = std(daily_log_returns, window=30) × sqrt(365)
```

| Trạng thái biến động (σ_ann) | Khoảng cách lưới (mỗi tầng) |
|---|---|
| Thấp (< 40%) | 1 – 2% |
...
1.  **HMM (Hidden Markov Model):** Chạy trên 3 hoặc 5 trạng thái dựa trên Log-returns và Realized Vol.
    - Sử dụng HMM 3 trạng thái làm mô hình cơ sở; tùy chọn mở rộng sang 5 trạng thái nếu độ chính xác kiểm chứng được cải thiện.
    - Các chỉ số cụm sẽ được lập bản đồ sau (post-hoc mapped) thành các trạng thái ngữ nghĩa dựa trên phân tích trọng tâm (centroid analysis).
...
> **[LƯU Ý v2.1]** Hệ thống định nghĩa 5 trạng thái ngữ nghĩa. Số lượng trạng thái HMM không nhất thiết phải bằng 5 — hãy sử dụng số lượng trạng thái tạo ra phân loại ổn định nhất (được xác thực qua AIC/BIC). Cần phải lập bản đồ ngữ nghĩa sau (post-hoc semantic mapping) từ các chỉ số cụm sang tên trạng thái sau mỗi lần huấn luyện lại.
...
state = [
    ...
    avg_cost_distance,           # Khoảng cách từ giá tới giá vốn (price - avg_cost_fifo_lot) / avg_cost_fifo_lot
...
**Định nghĩa về mức sụt giảm (Drawdown):** Phần trăm giảm giá trị danh mục so với giá trị danh mục lúc 00:00 UTC bắt đầu của cửa sổ rolling (24 giờ hoặc 7 ngày). Được đánh giá theo giá trị tương đương BTC (BTC-equivalent terms) tại mức giá BTC hiện tại.

| Kích hoạt (Trigger) | Hành động | [Giải thích lý do] |
|---|---|---|
| **Sụt giảm (Drawdown) > 15% trong 24h (so với ảnh chụp lúc 00:00 UTC)** | Tạm dừng các lệnh mua mới | Giá giảm quá sốc, cần chờ ổn định. |
| **Sụt giảm (Drawdown) > 25% trong 7 ngày (so với ảnh chụp lúc 00:00 UTC của 7 ngày trước)** | Tạm dừng toàn bộ giao dịch | Yêu cầu con người kiểm tra thủ công. |
...
| **Độ trượt khớp lệnh > 2% (so với giá mid tại thời điểm đặt lệnh)** | Dừng hệ thống, đối soát lại sổ cái | Đảm bảo tính toàn vẹn của dữ liệu giá vốn. |
...
## 19.2. Đo lường trượt giá (Slippage Measurement)

> **[SỬA ĐỔI v2.1]** Độ trượt giá được định nghĩa là độ lệch so với **giá mid tại thời điểm đặt lệnh**, chứ không phải từ giá giới hạn (limit price). Lệnh giới hạn được thực thi ở mức giá giới hạn hoặc tốt hơn, do đó việc đo lường so với giá giới hạn luôn bằng 0 — điều này là vô nghĩa.

```
slippage = |executed_price - mid_price_at_placement| / mid_price_at_placement
```

Điều này phản ánh chính xác tác động thị trường (market impact) và chi phí thời điểm (timing cost) cho cả lệnh giới hạn và lệnh thị trường.

## 19.3. Mô hình Thuế (Tax Model)
- Mỗi lệnh bán là một sự kiện tính thuế (Taxable event).
- Passive HODL chỉ đóng thuế khi bán hết vào cuối kỳ (Lợi thế thuế lớn).
- Thêm tham số `tax_drag` (có thể cấu hình theo khu vực tài phán; mặc định: áp dụng thuế suất ngắn hạn cho lợi nhuận đã thực hiện hàng năm).
- Backtest phải báo cáo cả lợi nhuận BTC **Trước thuế** và **Sau thuế**. Robot phải thắng được HODL ngay cả sau khi đã trừ thuế.

## 19.4. Yêu cầu báo cáo thuế (Tax Reporting Requirements)

> **[BỔ SUNG v2.1]** Sổ cái FIFO phải hỗ trợ xuất dữ liệu thuế. Các đầu ra bắt buộc bao gồm:

- Bản ghi chi tiết mỗi lần bán: `(sell_date, sell_price, sell_qty, lot_purchase_date, lot_purchase_price, realized_pnl_usd, holding_period_days)`
- Tóm tắt hàng năm: tổng lợi nhuận thực tế (ngắn hạn so với dài hạn dựa trên ngưỡng của khu vực tài phán)
- Định dạng xuất: CSV tương thích với các công cụ tính thuế crypto tiêu chuẩn (như Koinly, lược đồ CoinTracker)
- Tham số khu vực tài phán (Jurisdiction) trong cấu hình; mặc định giả định ngắn hạn = nắm giữ < 365 ngày

## 19.5. Phân tích độ nhạy (Sensitivity Analysis) - Bắt buộc
...
- **Cổng chặn bán FIFO:** Xác minh quy tắc `INV-5` sử dụng giá vốn ở cấp độ từng lô (lot-level), không dùng `avg_cost` toàn cầu.
...
- **Kiểm thử INV-3:** Đảm bảo `abs(sum(buckets) - total_portfolio) < EPSILON` trong mọi thao tác (bảo toàn vốn).
...
- **Cổng chặn bán:** Không bao giờ cho phép thực hiện một lệnh bán có giá vốn của lô lớn hơn giá bán sau khi đã trừ đi các khoản phí.
...
- **Các sản phẩm bàn giao (Deliverables):**
    1.  Bộ theo dõi giá vốn (FIFO, theo từng lô, sẵn sàng xuất báo cáo thuế).
...
- Chỉ chạy trên dữ liệu thời gian thực nhưng không dùng tiền thật.
- Thời gian thử nghiệm: Ít nhất 3 tháng (tốt nhất là 6 tháng).
- Theo dõi sát sao: Độ trễ mạng, mức độ trượt giá thực tế (so với giá mid tại thời điểm đặt lệnh) và chất lượng khớp lệnh so với mô hình lý thuyết.
